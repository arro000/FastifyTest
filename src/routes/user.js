...
const userSchema = {
  ...
  fastify.post("/login", userSchema, async (request, reply) => {
    ...
    if (ret.status) {
      ...
      const token=fastify.jwt.sign({user:body.username, roles:ret.object.roles})
  ...
  fastify.delete("/delete", deleteUserSchema,async (request, reply) => {
    ...
    if(request.user.roles.includes("admin")){
      ...
      if (ret.status) {
        ...
        return reply.code(200).send({ removed : ret.object[0].username });
      ...
      return reply.code(403).send({ message: "only admin can call this route" });
